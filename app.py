...
def create_patients_table(db):
    ...
    CREATE TABLE IF NOT EXISTS patients (
        ...
        address VARCHAR(255),
        date_added TIMESTAMP DEFAULT CURRENT_TIMESTAMPCHAR,
        email VARCHAR(255)
    )
    """
    ...
    CREATE TRIGGER before_insert_patients
        ...
        FOR EACH ROW
        SET NEW.date_added = IFNULL(NEW.date_added, CURRENT_TIMESTAMP);
    """
    create_procedure_query = """
        DELIMITER //
        CREATE PROCEDURE delete_and_archive_patient(
        IN patient_id INT
    )
    BEGIN
        -- Declare variables to store patient data
        DECLARE patient_name VARCHAR(255);
        DECLARE patient_age INT;
        DECLARE patient_contact_number VARCHAR(20);
        DECLARE patient_address VARCHAR(255);
        DECLARE patient_date_added TIMESTAMP;
        DECLARE patient_email VARCHAR(255);

        -- Retrieve patient data before deletion
        SELECT name, age, contact_number, address, date_added, email
        INTO patient_name, patient_age, patient_contact_number, patient_address, patient_date_added, patient_email
        FROM patients
        WHERE id = patient_id;
        ...
        -- Insert the deleted patient record into the archive table
        INSERT INTO archived_patients (
        name, age, contact_number, address, date_added, email, deletion_date
        ) VALUES (
            patient_name, patient_age, patient_contact_number, patient_address, patient_date_added, patient_email, CURRENT_TIMESTAMP
        );
    END //
    DELIMITER ;
    """
    create_archived_patients_query = """
    ...
    cursor.execute(create_archived_patients_query)
...
def delete_patient_record(db):
    ...
    cno = st.text_input("Enter patient's contact no:")
    query_arch = "INSERT INTO archived_patients SELECT * FROM patients WHERE contact_number = %s AND name = %s"
    
    query_del = "DELETE FROM patients WHERE contact_number = %s AND name = %s"
    button = st.button("Delete")
    if button:
        cursor.execute(query_arch,(cno,name,))
        cursor.execute(query_del,(cno,name,))
        db.commit()
        st.success("The patient record has been deleted")
...
def main():
    ...
    menu = ["Home","Add patient Record","Show patiet Records", "Search and Edit Patient","Delete Patients Record",
            "Add patients Appointments","Show All Appointments","Search and Edit Patients Appointments","Logout"]
    ...
    if options== "Home":
       ...
       if st.button("add patient record"):
        ...
        st.subheader("Search a patient to delete :")
         #delete_option = st.selectbox("Select delete option", ["ID", "Name", "Contact Number"])
         #delete_value = st.text_input("Enter delete value")
        delete_patient_record(db)
            #call_procedure_query = "CALL delete_and_archive_patient(%s)"
            #cursor.execute(call_procedure_query, (patient_id_to_delete,))
...
def check_login_credentials(username, password):
    try:
        ...
        query = "SELECT * FROM logincred WHERE username = %s AND password = %s"
...
def check_username_availability(username):
    try:
        ...
        # Execute the query to check username availability
        query = "SELECT * FROM logincred WHERE username = %s"
...
def update_password(username, old_password, new_password):
    try:
        ...
        query = "UPDATE logincred SET password = %s WHERE username = %s AND password = %s"
        ...
        if cursor.rowcount > 0:
            ...
            st.error("Wrong login credentials. Password not updated.")
...
def main_page():
    st.title("Hospital Management Application")
    action = st.radio("Select an action:", ("Login", "Signup", "Change Password"))
    ...
    if action == "Login":
        ...
        if st.button("Login"):
            if check_login_credentials(username, password):
                ...
                st.error("Wrong login credentials. Please try again.")
        ...
        if st.button("Signup"):
            if check_username_availability(username):
                ...
                cursor = conn.cursor()
                query = "INSERT INTO logincred (hospital_name, username, password) VALUES (%s, %s, %s)"
                cursor.execute(query, (hospital_name, username, password))
                conn.commit()
                cursor.close()
                conn.close()
